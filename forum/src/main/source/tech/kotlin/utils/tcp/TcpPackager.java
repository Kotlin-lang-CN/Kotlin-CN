...
import tech.kotlin.utils.encode.BytesEncoder;
...
public class TcpPackager {
     ...
     * 向管道中写TCP包
     */
    public int write(int type, byte[] data) throws IOException {
        byte[] sent = new byte[data.length + 8];
        mTagger.writeInt(sent, data.length, 0);
        mTagger.writeInt(sent, type, 4);
        System.arraycopy(data, 0, sent, 8, data.length);
        if (encoder != null) {
    ...
    public List<TcpPackage> fetch() throws IOException {
        ...
        while (mBuffer.getLen() > 0) {
            if (mBuffer.getLen() <= 8)//包头不完整
                break;

            mBuffer.peek(mTagger.mTagBuffer, 8);//取出包头
            int length = mTagger.readInt(mTagger.mTagBuffer, 0);
            int type = mTagger.readInt(mTagger.mTagBuffer, 4);

            if (mBuffer.getLen() < length + 8)//包内容未解析完整
                break;

            mBuffer.shift(8);//移位到包数据体开始
            ...
            dataList.add(new TcpPackage(type, data));
        }
        return dataList;
    }
    ...
    private static class Tagger {
        ...
        private byte[] mTagBuffer = new byte[8];
    ...
    private static class BufferRing {
        ...
        //写入方法
        void push(byte[] data) {
            if (length + data.length >= RING_BUF_SIZE)//当前环形队列已满，不能继续读包，当丢包处理
                return;
