...
import tech.kotlin.china.restful.component.Config
import tech.kotlin.china.restful.component.Database
...
open class _Rest {
     ...
     */
    val TOKEN_SECRET: String by lazy { config["token_secret"] }
    ...
    protected fun getToken(): TokenWrapper? = try {
        ...
        val decrypt = token.decrypt(TOKEN_SECRET)
    ...
    fun handleError(error: Throwable): Map<String, Any?> = when (error) {
        is BusinessError -> Maps.p("status", error.status).p("message", error.msg)
        is BindException -> Maps.p("status", 400).p("message", error.message)
        is ConversionNotSupportedException -> Maps.p("status", 500).p("message", error.message)
        is HttpMediaTypeNotAcceptableException -> Maps.p("status", 406).p("message", error.message)
        is HttpMediaTypeNotSupportedException -> Maps.p("status", 415).p("message", error.message)
        is HttpMessageNotReadableException -> Maps.p("status", 400).p("message", error.message)
        is HttpMessageNotWritableException -> Maps.p("status", 500).p("message", error.message)
        is HttpRequestMethodNotSupportedException -> Maps.p("status", 405).p("message", error.message)
        is MethodArgumentNotValidException -> Maps.p("status", 400).p("message", error.message)
        is MissingServletRequestParameterException -> Maps.p("status", 400).p("message", error.message)
        is MissingServletRequestPartException -> Maps.p("status", 400).p("message", error.message)
        is NoHandlerFoundException -> Maps.p("status", 404).p("message", error.message)
        is NoSuchRequestHandlingMethodException -> Maps.p("status", 404).p("message", error.message)
        is TypeMismatchException -> Maps.p("status", 400).p("message", error.message)
        is MissingPathVariableException -> Maps.p("status", 500).p("message", error.message)
        is NoHandlerFoundException -> Maps.p("status", 404).p("message", error.message)
        else -> Maps.p("stauts", 503).p("message", "未知服务器错误")
    }
}
